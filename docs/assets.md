...
Текущий первый экран использует цельную иллюстрацию, созданную и отредактированную встроенным ImageGen. У кондиционера удалены все черты лица, анатомия птицы исправлена до двух лап. Окно вертикально-подъёмное: нижняя створка поднята, проём под ней открыт, птица сидит на внешнем краю подоконника; комната, израильская застройка и вечерний свет сохранены. Это иллюстрация, а не фотография выполненной работы.

- `public/images/quiet-apartment-sash.webp` — сцена с подъёмным окном, 1536×1024.
- `public/images/quiet-apartment-sash-768.webp` — её мобильная версия, 768×512.
- Прежние `quiet-apartment.webp`, `quiet-apartment-768.webp`, `quiet-apartment-awake.webp`, `quiet-apartment-natural*.webp` и `quiet-apartment-open*.webp` — архивные варианты, больше не подключённые к странице.
...
Точные финальные промпты и режим генерации: [character-assets.json](character-assets.json). Изображения конвертированы в WebP через cwebp; версии с открытым и подъёмным окном — через Pillow. Персонаж и его реплики используются только на первом экране.
