...
Команда проверяет бизнес-данные, собирает сайт и выполняет `wrangler deploy`. При необходимости смените имя Worker в `wrangler.jsonc`. После подключения собственного домена пересоберите с новым `SITE_URL`. В пользовательский аккаунт Cloudflare автоматическая публикация не выполнялась.

Для Git-connected Worker: build `npm run build`, deploy `npm run deploy`, переменная сборки `SITE_URL`. Конфигурация `.openai/hosting.json` нужна только закрытому предпросмотру Sites и не требуется для вашего аккаунта Cloudflare.
